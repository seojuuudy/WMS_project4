...
<html lang="kr">
  <head>
	...
	<script type="text/javascript">
		function send_pr(product_cd, product_name, size_des, idx) {
// 			alert(idx);
    ...
    <div class="container-scroller">
      <div class="container-fluid page-body-wrapper">
       ...
       <div class="col-lg-12 grid-margin stretch-card">
                <div class="card">
                  <div class="card-body">
                   <h4 class="card-title">품목 검색</h4>
                    <p class="card-description">Product Search
                    	<form action="InboundScheduleSearchProduct.in">
						<!-- 검색 타입 추가 -->
			...
			<c:forEach var="product" items="${productList }">
				<tr>
					...
					<td>
						<a href="javascript:send_pr('${product.product_cd}', '${product.product_name}', '${product.size_des}', '${param.idx}')">
               	 ...
               	 <section id="pageList" style="text-align: center;"> <!-- 페이징 처리 영역 -->
						...
						<c:choose>
							<c:when test="${pageNum > 1}">
								<input type="button" class="btn btn-sm btn-outline-primary" value="이전" onclick="location.href='InboundScheduleSearchProduct.in?pageNum=${pageNum - 1}'">
							</c:when>
							<c:otherwise>
						...
						<c:forEach var="i" begin="${pageInfo.startPage }" end="${pageInfo.endPage }">
							<c:choose>
								...
								<c:otherwise>
									<a href="InboundScheduleSearchProduct.in?pageNum=${i }">${i }</a>
								</c:otherwise>
							</c:choose>
						</c:forEach>
						...
						<c:choose>
							<c:when test="${pageNum < pageInfo.maxPage}">
								<input type="button" class="btn btn-sm btn-outline-primary" value="다음" onclick="location.href='InboundScheduleSearchProduct.in?pageNum=${pageNum + 1}'">
							</c:when>
							<c:otherwise>
