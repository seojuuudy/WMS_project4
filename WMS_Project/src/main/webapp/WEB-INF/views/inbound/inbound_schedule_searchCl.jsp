...
<html lang="kr">
  <head>
    ...
    <script type="text/javascript">
    	function send_cl(business_no, cust_name) {
    		opener.set_cl(business_no, cust_name);
    		window.close();
		}
...
<%--         <jsp:include page="../partials/settings-panel.jsp"></jsp:include> --%>
       ...
       <div class="col-lg-12 grid-margin stretch-card">
                <div class="card">
                  <div class="card-body">
                    <h4 class="card-title">거래처 검색</h4>
<!-- 						<section id="buttonArea"> -->
					<p class="card-description">client Search</p>
					 <form action="InboundScheduleSearchClient.in">
						<!-- 검색 타입 추가 -->
...
<!-- 						</section> -->
                      ...
                      <table class="table table-striped">
						...
						<c:forEach var="client" items="${clientList }">
							<tr>
								...
								<td id="business_no">${client.business_no }</td>
								<td id="cust_name"><a href="javascript:send_cl('${client.cust_name }', '${client.business_no }')">${client.cust_name }</a></td>
		...
		<c:choose>
			<c:when test="${pageNum > 1}">
				<input type="button"  class="btn btn-sm btn-outline-primary" value="이전" onclick="location.href='InboundScheduleSearchClient.in?pageNum=${pageNum - 1}'">
			</c:when>
			<c:otherwise>
		...
		<c:forEach var="i" begin="${pageInfo.startPage }" end="${pageInfo.endPage }">
			...
			<c:choose>
				...
				<c:otherwise>
					<a href="InboundScheduleSearchClient.in?pageNum=${i }">${i }</a>
				</c:otherwise>
			</c:choose>
		</c:forEach>
		...
		<c:choose>
			<c:when test="${pageNum < pageInfo.maxPage}">
				<input type="button" class="btn btn-sm btn-outline-primary" value="다음" onclick="location.href='InboundScheduleSearchClient.in?pageNum=${pageNum + 1}'">
			</c:when>
			<c:otherwise>
