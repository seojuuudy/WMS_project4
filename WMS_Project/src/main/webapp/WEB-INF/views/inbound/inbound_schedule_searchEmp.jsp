...
<html lang="kr">
  <head>
	...
	<script type="text/javascript">
		function send_emp(emp_num, emp_name) {
			opener.set_emp(emp_num, emp_name);
			window.close();
		}
	</script>
  </head>
...
<%--         <jsp:include page="../partials/settings-panel.jsp"></jsp:include> --%>
       ...
       <div class="col-lg-12 grid-margin stretch-card">
           <div class="card">
              	<div class="card-body">
                    <h4 class="card-title">담당자 검색</h4>
                    <p class="card-description">employee Search</p>
					 <form action="InboundScheduleSearchEmployees.in">
						<!-- 검색 타입 추가 -->
...
<!--                             <th>부서</th> -->
                        ...
                        <c:forEach var="emp" items="${empList }">
                          <tr>
							<td id="emp_num">${emp.emp_num }</td>
                            <td id="emp_name"><a href="javascript:send_emp('${emp.emp_name}', '${emp.emp_num}')">${emp.emp_name}</a></td>
<%--                             <td>${emp.dept_name}</td> --%>
                          </tr>
                        </c:forEach>
...
<!-- 	           			<button type="button" class="btn btn-primary btn-rounded btn-fw" onclick="location.href='Regist.hr'">신규등록</button> -->
						...
						<c:choose>
							<c:when test="${pageNum > 1}">
								<input type="button" class="btn btn-sm btn-outline-primary" value="이전" onclick="location.href='InboundScheduleSearchEmployees.in?pageNum=${pageNum - 1}'">
							</c:when>
							<c:otherwise>
						...
						<c:forEach var="i" begin="${pageInfo.startPage }" end="${pageInfo.endPage }">
							<c:choose>
								...
								<c:otherwise>
									<a href="InboundScheduleSearchEmployees.in?pageNum=${i }">${i }</a>
								</c:otherwise>
							</c:choose>
						</c:forEach>
						...
						<c:choose>
							<c:when test="${pageNum < pageInfo.maxPage}">
								<input type="button" class="btn btn-sm btn-outline-primary" value="다음" onclick="location.href='InboundScheduleSearchEmployees.in?pageNum=${pageNum + 1}'">
							</c:when>
							<c:otherwise>
