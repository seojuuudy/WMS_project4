...
<!-- 	           			<button type="button" class="btn btn-primary btn-rounded btn-fw" onclick="location.href='Regist.hr'">신규사원 등록</button> -->
		...
		<c:choose>
			<c:when test="${pageNum > 1}">
				<input type="button"  class="btn btn-light" value="이전" onclick="location.href='ClientList.cl?pageNum=${pageNum - 1}'">
			</c:when>
			<c:otherwise>
		...
		<c:forEach var="i" begin="${pageInfo.startPage }" end="${pageInfo.endPage }">
			...
			<c:choose>
				...
				<c:otherwise>
					<a href="ClientList.cl?pageNum=${i }">${i }</a>
				</c:otherwise>
			</c:choose>
		</c:forEach>
		...
		<c:choose>
			<c:when test="${pageNum < pageInfo.maxPage}">
				<input type="button" class="btn btn-light" value="다음" onclick="location.href='ClientList.cl?pageNum=${pageNum + 1}'">
			</c:when>
			<c:otherwise>
