...
<!-- 			<h1>품목 조회</h1> -->	
       ...
       <div class="col-lg-12 grid-margin stretch-card">
                <div class="card">
                  <div class="card-body">
                    <h4 class="card-title">품목 조회</h4>
                    <p class="card-description"> Product List
                    	<form action="ProductSearchList.out">
						<!-- 검색 타입 추가 -->
               	 ...
               	 <section id="pageList" style="text-align: center;"> <!-- 페이징 처리 영역 -->
						...
						<c:choose>
							<c:when test="${pageNum > 1}">
								<input type="button" class="btn btn-sm btn-outline-primary" value="이전" onclick="location.href='ProductSearchList.out?pageNum=${pageNum - 1}'">
							</c:when>
							<c:otherwise>
						...
						<c:forEach var="i" begin="${pageInfo.startPage }" end="${pageInfo.endPage }">
							<c:choose>
								...
								<c:otherwise>
									<a href="ProductSearchList.out?pageNum=${i }">${i }</a>
								</c:otherwise>
							</c:choose>
						</c:forEach>
						...
						<c:choose>
							<c:when test="${pageNum < pageInfo.maxPage}">
								<input type="button" class="btn btn-sm btn-outline-primary" value="다음" onclick="location.href='ProductSearchList.out?pageNum=${pageNum + 1}'">
							</c:when>
							<c:otherwise>
