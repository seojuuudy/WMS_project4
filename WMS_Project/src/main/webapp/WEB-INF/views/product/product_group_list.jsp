			        ...
			        <div class="template-demo" style="text-align: right;">
						...
						<c:choose>
							<c:when test="${pageNum > 1}">
								<input type="button" class="btn btn-sm btn-outline-primary" value="이전" onclick="location.href='GroupList.pr?pageNum=${pageNum - 1}'">
							</c:when>
							<c:otherwise>
						...
						<c:forEach var="i" begin="${pageInfo.startPage }" end="${pageInfo.endPage }">
							<c:choose>
								...
								<c:otherwise>
									<a href="GroupList.pr?pageNum=${i }">${i }</a>
								</c:otherwise>
							</c:choose>
						</c:forEach>
						...
						<c:choose>
							<c:when test="${pageNum < pageInfo.maxPage}">
								<input type="button" class="btn btn-sm btn-outline-primary" value="다음" onclick="location.href='GroupList.pr?pageNum=${pageNum + 1}'">
							</c:when>
							<c:otherwise>
