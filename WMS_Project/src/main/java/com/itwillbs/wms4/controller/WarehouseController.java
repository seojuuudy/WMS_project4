...
//			int cpriv_cd = 19; // 관리권한 : 10011 부여 (19)
//			
...
//		}
	...
	public String joinPro(@ModelAttribute WarehouseVO warehouse, Model model) {
		System.out.println("컨트롤러 창고작업 : " + warehouse);
	...
	public String warehouseModify(String wh_cd, Model model, HttpSession session) { // 권한부여시 세션 넣기
		String sId = (String)session.getAttribute("sId");
	...
	// 사용 버튼 전용 값 출력 매핑
	@GetMapping("/UseAll.wh")
	public String useAll(
			@RequestParam(defaultValue = "") String searchType,
			@RequestParam(defaultValue = "") String keyword,
			@RequestParam(defaultValue = "1") int pageNum,
			@RequestParam(defaultValue = "1") String wh_use,
			Model model,
			HttpSession session) {
		int listLimit = 14; // 한 페이지에서 표시할 게시물 목록을 10개로 제한
		int startRow = (pageNum - 1) * listLimit; // 조회 시작 행번호 계산

		List<WarehouseVO> warehouseList = service.getUseWhList(searchType, keyword, startRow, listLimit, wh_use);
		int listCount = service.getBoardListCount(searchType, keyword);
		System.out.println("총 게시물 수 : " + listCount);
	...
	// 미사용 버튼 전용 값 출력 매핑
	@GetMapping("/NotUseAll.wh")
	public String NotUseAll(
			@RequestParam(defaultValue = "") String searchType,
			@RequestParam(defaultValue = "") String keyword,
			@RequestParam(defaultValue = "1") int pageNum,
			@RequestParam(defaultValue = "2") String wh_use,
			Model model,
			HttpSession session) {
		int listLimit = 14; // 한 페이지에서 표시할 게시물 목록을 10개로 제한
		int startRow = (pageNum - 1) * listLimit; // 조회 시작 행번호 계산
		
		List<WarehouseVO> warehouseList = service.getNotUseWhList(searchType, keyword, startRow, listLimit, wh_use);
		// 페이징 처리
		int listCount = service.getBoardListCount(searchType, keyword);
		System.out.println("총 게시물 수 : " + listCount);
